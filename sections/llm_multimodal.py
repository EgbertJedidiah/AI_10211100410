...
def llm_multimodal_section():
    st.header("🙋‍♂️LLM Q&A with Gemini AI")
    ...
    for i, name in enumerate(dataset_names):
        if cols[i].button(name):
            ...
            st.session_state.selected_dataset = name  # store selection in session

    # fallback to session state if exists
    if "selected_dataset" in st.session_state:
        selected_dataset = st.session_state.selected_dataset

    if selected_dataset:
        selected_path = datasets[selected_dataset]
        content = None
